...
const TableKendaraan = (props: Interface) => {
			...
			page.push(
				<div
					...
					onClick={() => {
						...
						controller
							.getDataKendaraan(i, controller.dataRow, "")
							.then(() => {});
					}}
				>
					{i + 1}
	...
	return (
		<div className={`flex-1 grid pb-10`}>
			<div className={`px-5`}>
				<div
					className={`border border-primary w-12 py-2 pl-1 flex place-items-center place-content-center rounded-lg cursor-pointer`}
					onClick={() => {
				...
				{controller.openRow ? (
					<div
						className={`flex-1 overflow-y-scroll scrollbar-hide py-1 shadow-lg bg-white rounded-lg h-40 absolute`}
					>
						...
						return (
							...
							>
								...
								>
									{controller.page == 0
										? index + 1
										: controller.dataRow * controller.page + (index + 1)}
								</td>
								<td
									className={`px-3 py-4 text-center whitespace-nowrap min-w-[60px]`}
								>
			...
			<div className={`mt-5 px-5 flex gap-2 place-content-end`}>
				<div
					...
					onClick={() => {
						if (controller.page >= 1) {
							...
							controller
								.getDataKendaraan(controller.page, controller.dataRow, "")
								.then(() => {});
						}
					}}
				>
				...
				<div className={`grid grid-cols-2 gap-2 tablet:flex`}>{page}</div>
				<div
					className={`w-20 h-10 flex place-content-center place-items-center rounded-lg border border-primary cursor-pointer hover:bg-primary hover:text-white`}
					onClick={() => {
